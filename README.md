...
3. 输入附加要求，设置目标篇幅基准值和 Mermaid 图示上限；系统会自动推导正文目标区间，并记住上次点击“开始扩写”时确认的这两个数值供下次直接带入。
4. 执行“开始扩写”后，右侧工作区会实时显示当前章节输出；若单章节启用事实卡片，当前引用关系会在扩写前自动保存。
...
- “新建配置...”会打开默认模板，需要填写项目根目录、投标主体名称，并选择大纲、采购需求、评分标准文件；保存后可切换到新保存的配置
