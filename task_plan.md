...
## 目标
- 评估当前本地桌面版切换为 CS 模式时的服务边界：把大模型调用、账号模型套餐、模型参数、系统门禁规则等集中到服务端 API 和数据库；尽量让生成后的文本处理、输出保存、合并和本地项目资料维护继续留在本地。

## 阶段
- [x] 梳理本地配置、模型调用、规则读取入口
- [x] 识别可上云与应留本地的职责边界
- [ ] 与用户确认服务端承载范围、账号体系和原始资料上云边界
- [ ] 写正式 CS 改造设计文档
- [ ] 拆分实施计划与接口契约

## 初步决策建议
- 首版不要把整套项目文件管理迁到服务端；本地继续管理大纲、采购需求、评分标准、事实卡片、输出正文、trace 与缓存。
- 服务端优先接管“模型路由与调用代理”：账号鉴权、套餐策略、模型参数、系统门禁规则、用量审计和调用日志。
- 本地客户端继续负责 prompt 组装与生成后处理；服务端只接收最终 messages 或标准化 generation request，并返回模型原始输出。
- `roles/system_gate_rules.md` 可以先迁到服务端数据库并由客户端启动时拉取缓存；角色文件 `writing.role_file` 是否也上云需要单独确认。
- embedding / verifier / outline / 正文生成这几类外部模型调用应统一经服务端转发，避免继续在客户端暴露不同 API key。

## 风险
- 如果服务端只代理模型调用但不参与 prompt 组装，服务端难以强制门禁规则确实进入 system prompt；需要让服务端能注入或校验 system gate rules。
- 如果把完整采购需求、评分标准和最终 prompt 都发到服务端，隐私边界会从“只给模型供应商”变成“服务器也持有项目敏感内容”；需要明确是否接受。
- 流式生成、长任务重试和本地 trace 对齐需要设计 request_id / trace_id，否则排查问题会比本地版更困难。
- 本地缓存和服务端模型参数更新之间需要版本号，否则同一章节重跑时可能因规则变更产生不可解释的差异。
...
## 目标
- 深入评估“在扩写当前章节时，把与其有依存关系的已写章节核心内容提炼后注入 prompt”的需求。
- 结合现有章节裁剪、prompt 拼装、已生成文件读取与 trace 机制，明确实现落点、方案分层、风险与推荐路径。

## 阶段
- [x] 复核现有文档与 prompt/裁剪机制，确认当前是否已有章节间依存注入
- [x] 复核代码中的 prompt 装配、章节写作计划、已生成章节读取能力
- [x] 归纳需求模型：依存关系类型、注入时机、摘要来源、配置边界
- [ ] 输出实现方案对比、推荐路径、分阶段实施建议

## 关键决策
- 当前评估先聚焦“章节扩写时的一致性增强”，不是重做整套检索架构。
- 优先基于“已写章节”的本地结果构建依存上下文，而不是直接依赖模型记忆或全文回看。

## 风险
- 如果依存章节选择不准，可能把错误约束注入后续章节，放大前文偏差。
- 如果摘要直接自由生成而没有结构约束，可能引入二次幻觉，反而降低一致性。

---

# config 项目文件结构评估与优化方案

## 目标
- 评估当前 `config*.yaml` 的结构混乱来源，而不是只看字段多少。
- 对照 `Config` 加载逻辑、README 推荐写法和现有项目配置，找出重复、歧义、未接线项和兼容包袱。
- 输出一套兼顾“继续兼容旧项目文件”和“后续能稳定维护”的优化方案。
