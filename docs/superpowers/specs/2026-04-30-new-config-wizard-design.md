...
- 新建项目时默认从选择招标文件开始。
- 根据招标文件所在目录推导项目根目录、配置文件路径、资料目录、大纲路径和输出目录。
- 对 `Downloads`、`Desktop`、临时目录等不适合作为项目根目录的位置，建议创建正式项目文件夹，并要求用户确认。
...
- 文件是否存在。
- 扩展名是否在支持范围内。
- 是否来自明显临时位置，例如 `Downloads`、`Desktop`、系统临时目录。
...
- 若招标文件位于普通业务目录，默认使用所在目录作为项目根目录。
- 若所在目录名称类似 `招标文件`、`项目资料`、`资料`、`采购文件`，优先建议其上一级作为项目根目录。
- 若招标文件来自 `Downloads`、`Desktop` 或临时目录，不直接用所在目录作为项目根目录；建议在当前配置所在目录旁或用户指定位置创建一个项目文件夹。
- 项目文件夹名优先从招标文件名去除“招标文件、采购文件、公告”等后缀得到；无法提取时使用“新项目”。
...
- 用户可从招标文件开始完成配置创建。
- 系统会推导并展示项目根目录，且对下载/桌面/临时目录给出新建项目文件夹建议。
- 项目外招标文件可复制到 `招标文件/`。
