...
候选排序需要优先真实章节标题：`第二章 采购服务要求`、`第四章 评标方法及评标标准` 这类标题应高于投标人须知正文或投标文件格式附件中对“采购需求”“评分标准”的引用。若关键词只是出现在较长说明句、偏离表格式说明或附件注释中，不应按完整目标章节候选处理。

DOCX 导入还需要区分目录和正文：目录中的 `第八章  货物需求一览表及技术规格\t67`、`第九章  评标方法和标准\t69` 只用于阅读，不参与正文边界；当正文 Word 标题省略章号，仅保留 `货物需求一览表及技术规格`、`评标方法和标准` 时，应使用标题层级切分真实章节。
