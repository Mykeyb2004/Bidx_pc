...
GUI 新建配置向导会先要求选择一个已存在的项目根目录。配置文件默认保存为 `project.root_dir/config_<项目文件夹名>.yaml`，配置文件选择器会以刚选择的项目根目录为初始目录；如果目录下已有 `.yaml/.yml` 配置文件，用户可以显式选择该文件作为保存目标，保存时按配置编辑器规则创建备份。保存后的 YAML 使用 `project.root_dir: "."`。采购需求、评分标准、大纲、节点撰写计划和输出目录的默认值都从该项目根目录派生。选择招标文件只会更新来源文件、复制目标和导入目录，不会重新推导或覆盖项目根目录。
