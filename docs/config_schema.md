...
- 新建配置默认使用 `./投标大纲.md` 作为大纲文件路径；如果该路径或用户选择的 Markdown 文件已经存在，保存时会将 `project.outline_locked` 写为 `true` 并复用该文件；如果路径不存在，则写为 `false`，保存并应用后进入大纲准备窗口生成到该路径
...
GUI 新建配置向导会先要求选择一个已存在的项目根目录。配置文件默认保存为 `project.root_dir/config_<项目文件夹名>.yaml`；“选择...”会以刚选择的项目根目录为初始目录，并打开已有 `.yaml/.yml` 配置文件，不显示另存为或覆盖提示。需要新文件名时可直接修改默认保存路径。保存时按配置编辑器规则创建备份。保存后的 YAML 使用 `project.root_dir: "."`。采购需求、评分标准、大纲、节点撰写计划和输出目录的默认值都从该项目根目录派生；大纲文件选择器始终只打开已有 `.md` 文件，不使用另存为对话框。若大纲路径已存在则复用，若路径不存在则在保存并应用后进入大纲准备窗口生成。选择招标文件只会更新来源文件、复制目标和导入目录，不会重新推导或覆盖项目根目录。
...
- `false`：新建配置的大纲路径尚不存在，GUI 会先打开“大纲准备”窗口并在确认后写入该路径。
...
# 可选：none / minimal / low / medium / high / xhigh；未设置时使用模型默认值
