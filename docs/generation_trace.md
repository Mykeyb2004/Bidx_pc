...
当大纲准备窗口启用生成时，也会先展示阶段状态，再在模型开始返回内容后流式写入编辑框；这类阶段提示属于 UI 交互反馈，不会单独写入 trace 文件，但 trace 仍会保留最终 prompt、请求参数和最终输出正文，方便回看生成过程。
...
- `scope`：固定为 `h2`
- `summary_chars`：历史字段名，当前表示项目背景材料字符数；`content_mode=summary` 时才是摘要长度
- `cache_status`
- `fallback_reason`
- H2 背景专属字段：`h2_title`、`h2_full_path`、`evidence_unit_ids`、`evidence_blocks`、`evidence_count`、`precomputed`

H2 背景的 `evidence_blocks` 是采购需求原文片段。默认 `content_mode=excerpts` 时，项目背景正文即由这些片段格式化而来；`summary` 模式下则用于人工审计摘要是否有依据。
