...
`repair_numbering()` 优先整体提升标题树并重编号；纯 Markdown 标题可转换为正式层级。只删除开头与当前输入标题精确匹配的重复行（比较时忽略 Markdown/加粗包裹和行尾换行装饰）。混排等不明确情况最多请求一次当前模型（禁用 SDK 重试），仅返回全部候选标题的原始行号及 1–4 级结构。程序严格验证建议，从原始标题文字构造编辑，普通正文、数据、表格及代码保持原样。标题完全缺失或围栏未闭合时无法可靠修复。
...
编号修复不重新扩写全文，也不凭空创建缺失标题。标题边界或父子关系不能可靠判定时保留草稿；模型结构建议仍需程序复核。流式原始输出可能短暂出现错误编号，只有最终合格结果才会自动保存。非编号文风问题仍仅记录诊断。底层模型请求取消后可能持续到连接超时，但迟到结果不会用于保存。
