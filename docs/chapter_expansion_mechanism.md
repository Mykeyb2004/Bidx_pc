...
`body_layout.prepare_layout()` 先恢复确定的标题之间、加粗正式标题与正文、表名与表头的边界，只插入换行。标题与正文难以分界时，模型只返回候选原行精确前缀及层级；疑似普通枚举可选择保留原行。断行前后行号由程序映射，模型不能改写标题或正文。零个独立标题但存在粘连候选时也允许修复；长冒号引导句不因已被旧解析器认作标题而被漏过。
...
Mermaid 完全排除在修复范围外。`protected_code_lines()` 识别合法围栏以及“短图名＋粘连 Mermaid 开围栏”，保护到独立闭围栏，以便继续检查图后的标题；代码、连线、语言标记、围栏及粘连图名均不修改，主体称谓替换也跳过代码。不做 Mermaid 语法检查；真正缺少闭围栏或没有任何标题线索时仍保留原稿。
...
结构与编号修复不重新扩写全文，也不凭空创建缺失标题。模型可选择原文前缀恢复断行，边界或父子关系无法可靠判定时保留草稿；结构建议仍需程序复核。流式原始输出可能短暂出现粘连或错误编号，只有最终正文结构合格才会自动保存。Mermaid 原样保留，不以此流程保证图语法有效。非编号文风问题仍仅记录诊断。底层模型请求取消后可能持续到连接超时，但迟到结果不会用于保存。
