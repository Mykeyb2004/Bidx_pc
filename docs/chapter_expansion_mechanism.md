...
6. GUI 后台线程在接收完成后调用 `AIWriter.finalize_generation()`，检查编号、本地修复，必要时请求一次模型结构判断，再复核
7. 复核通过后 `FileSaver.save()` 将章节正文保存为本地 Markdown 文件；失败保留草稿

关键入口文件：

- `bid_writer/gui.py`
- `bid_writer/ai_writer.py`
- `bid_writer/body_numbering.py`
...
- 大纲准备窗口同样会先显示阶段状态；如果大纲模型开始返回内容，文本会立即流式写入编辑框，因此即使首批内容较慢，用户也能持续看到进度
...
- 全局事实卡片自动参与启用事实卡片模式的扩写；批量汇总按每个章节的有效引用结果统计，而不是按卡片列表的默认勾选状态推断
...
`roles/system_gate_rules.md` 在独立 system message 中统一要求：

- 正文不要写成无序号散文
- 首个正文标题必须从 `一、` 开始，输入大纲编号不占正文内部层级
- 最多使用 `一、 → （一） → 1. → （1）` 四层，按需要展开，同级连续、下级随父级重置
- 禁止 Markdown 标题，不禁止正式一级标题；禁止重复输入章节标题
- 序号后若带标题，该行只写“序号 + 标题”，正文另起

user prompt 只提醒遵守 system 门禁，不再复制独立结构约束区块。角色设定、节点计划或参考材料不能覆盖上述格式规则。

### 3.5 额外规则

- `prompt.extra_rules` 不再单独成段，而是追加到 `## 输出硬约束提醒` 的末尾

该项属于 user prompt 的结构补充，不属于 system 级约束。

### 3.6 事实卡片如何进入 prompt

- 全局卡片：`active=true` 且 `scope=global` 时，在单章节生成参数窗口中默认勾选并排在列表最前面；用户可按章节取消，取消状态会保存到该章节事实卡片引用关系的 `selections`
...
- 批量模式：先读取各章节已保存的 `should_reference`；若本章明确为 `false` 则不注入事实卡片，否则默认加入全局卡片、排除各章节已保存的全局取消项，并读取各章节已保存的局部事实卡片引用关系。参数窗口同时展示模式启用数和每张卡片的有效引用数
- 批量统一应用：独立配置窗口只提交用户明确选择的补丁；全局卡片“全部引用”会删除章节显式排除，“全部排除”会写入 `selected: false`；局部卡片“全部引用”写入选中，“全部排除”删除选中。未选择“保持各章节”的卡片和关闭模式时的既有选择均保留；所有章节在内存中更新后只保存一次 YAML
- 多选直接生成不会触发批量保存；“本次批量忽略全部事实卡片”仅把本次 `fact_card_mode` 设为 `false`，不会改变 `fact_cards.chapter_defaults`
...
2. `expand_raw()`
3. 接收完成后在同一后台线程执行 `finalize_generation()`，校验通过后才发送 `done`

这意味着：

- 主窗口右侧正文工作区中实时显示的是原始模型输出
- 后处理发生在流式输出结束后，通过队列替换工作区正文，不阻塞主线程；模型结构判断可取消
...
- 先做编号检查、修复、复核和主体称谓归一化
- 合格后自动保存到输出目录；显示“已修复正文编号并保存”或“已自动保存”
- 无法修复时保留原始草稿，不自动保存、不覆盖旧文件，也不提炼事实卡片；批量生成记为失败后继续
...
后处理先保证正文编号合格，再沿用主体称谓归一化。`AIWriter.expand()` 的流式/非流式接口与 GUI 共用同一个最终校验入口。
...
`body_numbering.inspect_numbering()` 检查首标题 `一、`、Markdown 标题、标题层级跳级、同级连续编号及下级重置。只编辑可辨认的独立短标题（不含句末、分号等正文标点），跳过管道表格、引用及围栏代码；不把长段落枚举当作可编辑标题。

独立加粗短标题和 `①`–`⑳` 圈号短标题也作为可修复候选，检测结果为 `nonstandard_heading`。这些样式没有确定层级，内部 `level=0` 表示未知；只通过一次模型建议转换为 1–4 级，不按字体样式推断父子关系。标题后的括号注释、行尾两个空格或反斜杠、LF/CRLF 原样保留。加粗的普通完整句子、行内多项圈号枚举不作为标题。

`body_layout.prepare_layout()` 先恢复 Mermaid 代码块边界，再恢复确定的各层级标题之间（包括下级转上级）、加粗正式标题与正文、表名与表头的边界，只插入换行。标题与正文难以分界时，模型只返回候选原行精确前缀及层级；疑似普通枚举可选择保留原行。断行前后行号由程序映射，模型不能改写标题或正文。零个独立标题但存在粘连候选时也允许修复；长冒号引导句不因已被旧解析器认作标题而被漏过。

`repair_numbering()` 随后整体提升标题树并重编号；纯 Markdown 标题可转换为正式层级。只删除开头与当前输入标题精确匹配的重复行（比较时忽略 Markdown/加粗包裹和行尾换行装饰）。边界判断与混排层级判断合用最多一次当前模型请求（禁用 SDK 重试），全部候选必须按原行序覆盖。原始文字、数据及表格单元格不改动。

Mermaid 仅本地修复明确的换行边界：图名前后分段、围栏独立成行、语言标记与声明分行、flowchart/graph 方向与首条语句分行，以及 `G --> F F --> I` 这样的完整连线拆行。`FF` 完整节点标识、图内标签、引号及注释保持原样，不修改任何非换行字符。支持代码块、四层标题和正文全部粘在同一行，先分离代码再继续处理图外标题；多个图示也依次处理。图内不确定语句不拆改，不进行完整语法校验或新增模型调用。修复后的代码区域仍受保护，主体称谓替换跳过代码；真正缺少闭围栏或没有任何标题线索时仍保留原稿。

修复结果必须再次通过检查，否则抛出 `BodyNumberingError`，携带原文和问题明细。超时、取消、无效响应不重复请求。`format_repair_applied` 仅在修复被接受时为 true，修复方法及前后问题列表记入 postprocess 和 trace。
...
- 不会为了消除这些文风问题而重写正文；编号门禁由上述专用模块处理
...
结构与编号修复不重新扩写全文，也不凭空创建缺失标题。模型可选择原文前缀恢复断行，边界或父子关系无法可靠判定时保留草稿；结构建议仍需程序复核。流式原始输出可能短暂出现粘连或错误编号，只有最终正文结构合格才会自动保存。Mermaid 仅恢复明确的换行边界，保留节点与连接关系，不以此流程保证全部图语法有效。非编号文风问题仍仅记录诊断。底层模型请求取消后可能持续到连接超时，但迟到结果不会用于保存。
