...
- 批量模式逐章执行
- 顶部“生成所选”按钮与“终止生成”共用同一个控件；生成开始后按钮立即切换为“终止生成”，点击后当前章节继续完成但不保存，前面已经保存的章节保留，后续章节不再启动
...
- 多选保存：生成参数窗口允许把当前事实卡片选择批量保存到所选章节；该操作会覆盖这些章节已有的事实卡片引用关系，保存后批量生成即可按新的章节关系读取
