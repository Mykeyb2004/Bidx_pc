...
编辑器不编辑这些连接信息；连接检测仅作为内部校验和加载配置时的辅助信息使用。

### 2.4 保持编辑区聚焦

配置表单以左侧分组导航和中间表单为主，不再固定展示连接状态、配置摘要、校验结果和 YAML 预览。校验仍在保存时执行，未保存状态仍在窗口顶部提示。
...
| 配置分组         | 当前分组表单                                                                                                  |
|------------------|---------------------------------------------------------------------------------------------------------------|
| 项目             | 项目根目录   [ /path/to/project                                                               ][浏览]        |
| 写作             | 投标主体名称 [ 杭州菲尔德咨询                                                                   ]             |
| 处理路径         | 大纲文件     [ ./投标大纲.md                                                                    ][浏览]        |
| 运行             | 采购需求文件 [ ./采购需求.md                                                                    ][浏览]        |
|                  | 评分标准文件 [ ./评分标准.md                                                                    ][浏览]        |
|                  | 输出目录     [ ./output                                                                         ][浏览]        |
+------------------+---------------------------------------------------------------------------------------------------------------+
| [恢复加载值] [恢复默认值]                                                         [另存为] [保存]              |
