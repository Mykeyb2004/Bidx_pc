...
当前项目背景由 `bid_writer/project_background.py` 中的 `ProjectBackgroundGenerator` 负责：它读取完整采购需求，生成一次全局项目背景摘要，并按源文与 `max_chars` 缓存。`AIWriter.build_prompt_result()` 在项目背景启用时，会把该全局背景注入到章节 prompt 中。

这套机制能减少直接注入完整采购需求的压力，但在 `processing.path: auto` 下存在明显问题：

- `auto` 已经是章节级上下文裁剪链路，核心目标是让当前章节只接收相关的需求、评分和边界信息。
- 全局项目背景会进入每个章节 prompt，容易把与当前 H2 无关的项目范围、交付物或质量要求带入章节扩写。
- 如果采购需求很长，全局摘要为了覆盖全面会变得抽象，反而弱化当前 H2 下章节扩写所需的项目情境。
- 直接减少全局背景长度又有风险：可能丢掉当前 H2 下必须保留的关键业务背景。

因此，本优化目标不是简单压缩项目背景，而是把“项目背景”从全局摘要调整为 `auto` 模式下的 H2 级、证据驱动、可追溯背景摘要。
...
- 背景摘要必须基于采购需求原文检索结果生成，不能凭模型自由概括。
...
`full_context` 模式仍使用当前全局 `ProjectBackgroundGenerator`，因为 full-context 的定位就是完整参考，不需要强制改为 H2 背景。

单章节手动扩写时，如果对应 H2 缓存不存在，可按配置选择“先为该 H2 补生成一次”或“回退到全局背景/空背景”。批量扩写路径应默认先执行全量 H2 背景预生成，避免每个叶子章节扩写时重复触发摘要调用。
...
- `scope: global` 保持当前行为。
- `scope: h2_auto` 表示仅在 `auto` 下使用 H2 背景；非 auto 仍可回退 global。
- `precompute_on_batch` 表示批量扩写前先一次性生成所有 H2 背景缓存，建议默认开启。
- `generate_missing_on_single` 表示单章节扩写时若 H2 缓存缺失，是否先补生成当前 H2 背景；建议默认开启。
- `include_evidence_in_prompt` 首版建议默认 `false`，避免 prompt 变长；但 evidence 必须进入 trace。
...
5. 若配置 `fallback=global`，使用当前全局 `ProjectBackgroundGenerator`。
6. 若配置 `fallback=raw_evidence`，把命中的 1-3 个采购需求片段作为短背景依据注入。
7. 若配置 `fallback=empty`，不注入项目背景。

默认建议 `fallback=global`，因为它对现有行为最兼容。
...
- `h2_project_background_include_evidence_in_prompt`
...
`跳过并回退` 应按配置的 `fallback` 执行。默认 `fallback=global` 时，正文扩写继续使用全局项目背景。
...
```text
H2ProjectBackgroundGenerator.get_or_generate
```
...
| 风险 | 表现 | 缓解 |
|---|---|---|
| H2 检索召回不足 | 背景摘要过泛或缺关键事实 | 设置 evidence 下限，低召回回退 global/raw evidence |
| 摘要幻觉 | 出现采购需求没有的信息 | prompt 限制只基于证据；trace 保留 evidence；温度设为 0 |
| 同名 H2 缓存冲突 | 不同章节复用错误背景 | cache key 使用 `h2.full_path` 和 subtree hash |
| prompt 变长 | 摘要加证据后抵消优化收益 | 默认不把 evidence 注入 prompt，只进 trace |
