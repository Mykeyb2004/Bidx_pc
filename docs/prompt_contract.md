...
`full_context` 会把完整采购需求放入 prompt；当 `processing.scoring.enabled=true` 时同时放入完整评分标准。该分支不会额外生成 `project_background`。
...
| `scoring_focus` | `## 评分关注` | pruned/auto 分支、评分链路启用且命中评分项时 |
| `fact_card_context` | `## 事实卡片参考` | 启用事实卡片且本章有可用卡片时 |
| `bid_requirements` | `## 招标需求参考` | full_context 分支且有采购需求原文时 |
| `scoring_criteria` | `## 评分标准参考` | full_context 分支、评分链路启用且有评分标准原文时 |
...
`full_context` 分支会根据是否存在完整采购需求和已启用的评分标准，改成引用前文固定参考材料，避免“下方”指代错位。
...
评分检索可以走规则表格解析，也可以走 hybrid retrieval。若开启 verifier，辅助模型只返回候选 ID，程序再回填原文。`processing.scoring.enabled=false` 时，本节对应的检索、分类和 prompt 注入都会跳过。
