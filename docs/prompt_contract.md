...
先由 `body_layout.prepare_layout()` 调用本地 Mermaid 换行修复，再恢复正文边界：本地分开有明确序号关系的粘连标题、有明确加粗包裹的正式标题与正文，以及表名与合法表头；后者要求下一行是列数一致的分隔行。只插入换行，不改业务文字或表格单元格。疑似标题贴正文的正式序号段落进入边界判断，已有部分正常标题不豁免其他粘连行。长冒号引导句即使被旧解析器认作短标题，也进入候选判断。

边界不明确、标题混排或跳级时最多请求一次当前模型，禁用 SDK 重试。普通标题仍使用 `{"headings":[{"line":1,"level":1}]}`；存在边界候选时，一次响应同时覆盖 `editable_headings` 与 `boundary_candidates`，候选项增加 `prefix`，必须是包括序号的原行精确前缀，程序只在其后插入换行。不接受替换正文、额外字段、缺失/重复/乱序行号或不合法层级。确属普通段内枚举时，候选可返回 `level=null,prefix=null` 保留原行；`standalone=true` 的候选若整行确实为标题，可选择完整原行不作断行。断行后重新映射行号，再由编号模块整体提升层级、连续重编号、转换 Markdown 标题、删除与已知输入标题精确匹配的开头重复标题。边界与层级判断共享一次请求预算。

`mermaid_layout.repair_mermaid_layout()` 仅补齐可明确识别的换行：代码块与图名及前后正文分段、语言标记与图声明分行、flowchart/graph 方向与首条语句分行、闭围栏独立成行；`G --> F F --> I` 这类完整连线由空格粘连时拆成两行，`G --> FF --> I` 保留。所有非换行字符（包括原空格）、节点和连接关系不改动；节点标签、引号、注释、其他代码块与不确定语句不拆改。不执行 Mermaid 渲染或完整语法校验。图前、图后各层级标题及正文即使全在同一行，也在代码边界恢复后继续由标题修复流程处理；明确的下级标题接上级标题也可分行。没有闭围栏时不补写围栏。恢复后的代码区域仍禁止模型编辑，主体称谓归一化也跳过代码。system 门禁同步要求图示与标题正文分段、独立语句分行。
...
修复后复核通过才返回成功结果。失败抛出携带原始正文的 `BodyNumberingError`，GUI 保留未保存草稿、不覆盖旧文件，批量任务记为失败后继续。零个独立标题但有粘连线索时允许进入边界判断；既无标题又无线索的纯正文直接保留草稿。取消、超时及无效 JSON 不触发再次请求。流式原始文本可能短暂展示粘连或错误编号，最终结果会整段替换为通过复核的正文；此成功状态不表示 Mermaid 语法有效。
