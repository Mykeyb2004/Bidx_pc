...
先由 `body_layout.prepare_layout()` 恢复正文边界：本地分开有明确序号关系的粘连标题、有明确加粗包裹的正式标题与正文，以及表名与合法表头；后者要求下一行是列数一致的分隔行。只插入换行，不改业务文字或表格单元格。疑似标题贴正文的正式序号段落进入边界判断，已有部分正常标题不豁免其他粘连行。长冒号引导句即使被旧解析器认作短标题，也进入候选判断。
...
Mermaid 不参与修复：代码内容、声明、连线、语言标记、围栏，以及粘在开围栏前的图名均原样保留。扫描器识别“短图名＋Mermaid 开围栏”到独立闭围栏之间的受保护区域，以免错误屏蔽图后正文，但不校验或修正 Mermaid 语法。主体称谓归一化也跳过代码区域。真正没有闭围栏时，区域边界不可靠，继续保留原稿而非补写围栏。
